...
public class RivenWidget {
    ...
    private final RivenTab tab;
    private final RivenSlotInfo advancement;
    private final FormattedCharSequence title;
    private final int width;
    private final List<FormattedCharSequence> description;
    private final Minecraft minecraft;
    @Nullable
    private RivenWidget parent;
    private final List<RivenWidget> children = Lists.newArrayList();
    @Nullable
    private AdvancementProgress progress;
    private final int x;
    private final int y;

    public RivenWidget(RivenTab p_97255_, Minecraft p_97256_, RivenSlotInfo slotInfo) {
        this.tab = p_97255_;
        this.advancement = slotInfo;
        this.minecraft = p_97256_;
        this.title = Language.getInstance().getVisualOrder(p_97256_.font.substrByWidth(slotInfo.getTitle(), 163));
        this.x = Mth.floor(slotInfo.getX() * 28.0F);
        this.y = Mth.floor(slotInfo.getY() * 27.0F);
        int i = 1;
        ...
        this.description = Language.getInstance().getVisualOrder(this.findOptimalLines(ComponentUtils.mergeStyles(slotInfo.getLore().copy(), Style.EMPTY.withColor(slotInfo.getFrameType().getChatColor())), l));
    ...
    public RivenSlotInfo getAdvancement() {
        return advancement;
    }
    ...
    @Nullable
    private RivenWidget getFirstVisibleParent(RivenSlotInfo p_97312_) {
        p_97312_ = p_97312_.getParent();

        return p_97312_ != null ? this.tab.getWidget(p_97312_) : null;
    }
    ...
    public void drawConnectivity(GuiGraphics p_281947_, int p_97300_, int p_97301_, boolean p_97302_) {
        ...
        for(RivenWidget rivenWidget : this.children) {
            rivenWidget.drawConnectivity(p_281947_, p_97300_, p_97301_, p_97302_);
        }

    }

    public void draw(GuiGraphics p_281958_, int p_281323_, int p_283679_) {
            if (!this.advancement.isHidden() || this.progress != null && this.progress.isDone()) {
                ...
                p_281958_.blit(WIDGETS_LOCATION, p_281323_ + this.x + 3, p_283679_ + this.y, this.advancement.getFrameType().getTexture(), 128 + advancementwidgettype.getIndex() * 26, 26, 26);
                p_281958_.renderFakeItem(this.advancement.getItem(), p_281323_ + this.x + 8, p_283679_ + this.y + 5);
            }

        for(RivenWidget rivenWidget : this.children) {
            rivenWidget.draw(p_281958_, p_281323_, p_283679_);
        }
    ...
    public void addChild(RivenWidget rivenWidget) {
        this.children.add(rivenWidget);
    }

    public void drawHover(GuiGraphics graphics, int p_281304_, int p_281253_, float p_281848_, int p_282097_, int p_281537_) {
        ...
        if(advancement.isSlot() && advancement.getItem() != ItemStack.EMPTY)  {
            graphics.renderTooltip(minecraft.font, advancement.getItem(), p_281304_ + this.x + 8, p_281253_ + this.y + 5);
        }else {
            ...
            graphics.blit(WIDGETS_LOCATION, p_281304_ + this.x + 3, p_281253_ + this.y, this.advancement.getFrameType().getTexture(), 128 + advancementwidgettype2.getIndex() * 26, 26, 26);
            if (flag) {
                graphics.drawString(this.minecraft.font, this.title, i1 + 5, p_281253_ + this.y + 9, -1);
                if (s != null) {
                    graphics.drawString(this.minecraft.font, s, p_281304_ + this.x - i, p_281253_ + this.y + 9, -1);
                }
            } else {
                graphics.drawString(this.minecraft.font, this.title, p_281304_ + this.x + 32, p_281253_ + this.y + 9, -1);
                if (s != null) {
                    graphics.drawString(this.minecraft.font, s, p_281304_ + this.x + this.width - i - 5, p_281253_ + this.y + 9, -1);
                }
            }
            ...
            if (flag1) {
                ...
                for (int l1 = 0; l1 < this.description.size(); ++l1) {
                    graphics.drawString(this.minecraft.font, this.description.get(l1), i1 + 5, p_281253_ + this.y + 9 + 17 + l1 * 9, -5592406, false);
                }
            }

        }
        graphics.renderFakeItem(this.advancement.getItem(), p_281304_ + this.x + 8, p_281253_ + this.y + 5);
    }
